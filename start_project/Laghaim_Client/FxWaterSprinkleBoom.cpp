...
CFxWaterSprinkleBoom::CFxWaterSprinkleBoom()
{
	m_fTailSize = 3.0f;					// fireball의 꼬리 잔상 텍스처이다.
	m_fSpeed = 8.0f;
	m_fBoomSize = m_fTailSize * 5.0f;
...
void CFxWaterSprinkleBoom::LoadRes(IndexedTexture *pIndexedTexture, CShadowRect *pShadowRect)
{}

void CFxWaterSprinkleBoom::DeleteRes()
{}

BOOL CFxWaterSprinkleBoom::Render(D3DVECTOR &vPosFrom, D3DVECTOR &vPosTo, int nFrame)
{
	if( !GET_D3DDEVICE() )
		return true;
	...
	if (bDrawTail)
	{
		...
		if( g_pCapsyongTexture )
			lpSurface = g_pCapsyongTexture->GetSurfaceByIdx( g_EffectTextr[ EFF_FX_WTRSPRINKLEBOOMTAIL ] );
	...
	matWorld._42 = vPosTo.y + FX_WTRSPRINKLE_HEIGHT;
	matWorld._43 = vPosTo.z;

	MakeLVertexRect(m_lverBoom, m_fBoomSize, m_fBoomSize, TRUE);

	if (nFrame < FX_WTRSPRINKLEBOOM_FRAMECOUNT)
	{
	...
	// nate 2004 - 7 - image
	if( g_pCapsyongTexture )
		lpSurface = g_pCapsyongTexture->GetSurfaceByIdx( g_EffectTextr[ EFF_FX_WTRSPRINKLEBOOM ] );
	GET_D3DDEVICE()->SetTexture(0, lpSurface);
	...
	if (nFrame >= FX_WTRSPRINKLEBOOM_FRAMECOUNT - 1)
		bFinished = TRUE;
