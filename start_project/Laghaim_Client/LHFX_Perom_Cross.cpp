...
const int male_frame = 36;
const int fmale_frame = 32;

LHFX_Perom_Cross::LHFX_Perom_Cross(void)
	:	m_lolo(NULL)
{
}
...
void LHFX_Perom_Cross::LoadRes()
{
	if (!m_lolo)
	{
		m_lolo = new CLolos[2];
		m_lolo[0].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[0].Read("data/effect/lolos/pm_skill_cross.lol");
		m_lolo[1].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[1].Read("data/effect/lolos/pw_skill_cross.lol");
	}
}

void LHFX_Perom_Cross::DeleteRes()
{
}

bool LHFX_Perom_Cross::Render(EffectSort &effect_sort)
{
