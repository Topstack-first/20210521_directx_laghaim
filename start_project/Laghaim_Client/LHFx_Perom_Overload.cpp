...
const int fmale_frame = 43;

LHFx_Perom_Overload::LHFx_Perom_Overload(void)
	:	m_lolo(NULL)
{
}
...
void LHFx_Perom_Overload::LoadRes()
{
	if (!m_lolo)
	{
		m_lolo = new CLolos[2];
		m_lolo[0].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[0].Read("data/effect/lolos/pm_skill_overload_ap.LOL");
		m_lolo[1].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[1].Read("data/effect/lolos/pw_skill_overload_ap.LOL");
	}
}

void LHFx_Perom_Overload::DeleteRes()
{
}

bool LHFx_Perom_Overload::Render(EffectSort &effect_sort)
{
