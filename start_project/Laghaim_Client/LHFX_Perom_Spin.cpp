...
const int male_frame = 22;
const int fmale_frame = 22;

LHFX_Perom_Spin::LHFX_Perom_Spin(void)
	:	m_lolo(NULL)
{
}
...
void LHFX_Perom_Spin::LoadRes()
{
	if (!m_lolo)
	{
		m_lolo = new CLolos[2];
		m_lolo[0].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[0].Read("data/effect/lolos/pm_skill_spin.lol");
		m_lolo[1].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[1].Read("data/effect/lolos/pw_skill_spin.lol");
	}
}

void LHFX_Perom_Spin::DeleteRes()
{
}

bool LHFX_Perom_Spin::Render(EffectSort &effect_sort)
{
