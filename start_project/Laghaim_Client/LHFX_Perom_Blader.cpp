...
const int male_frame = 56;
const int fmale_frame = 56;

LHFX_Perom_Blader::LHFX_Perom_Blader(void)
	:	m_lolo(NULL)
{
}
...
void LHFX_Perom_Blader::LoadRes()
{
	if (!m_lolo)
	{
		m_lolo = new CLolos[2];
		m_lolo[0].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[0].Read("data/effect/lolos/pm_skill_blader.lol");
		m_lolo[1].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[1].Read("data/effect/lolos/pw_skill_blader.lol");
	}
}

void LHFX_Perom_Blader::DeleteRes()
{
}

bool LHFX_Perom_Blader::Render(EffectSort &effect_sort)
{
