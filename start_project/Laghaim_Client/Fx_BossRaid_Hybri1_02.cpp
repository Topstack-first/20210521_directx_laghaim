...
CFxBossRaidHybri1_02::CFxBossRaidHybri1_02()
{
	m_pLolo = NULL;
}
...
void CFxBossRaidHybri1_02::LoadRes()
{
	if (!m_pLolo)
	{
		m_pLolo = new CLolos;
		m_pLolo->SetIndexedTexture( g_pCapsyongTexture );
		m_pLolo->Read("data/effect/lolos/knife_energy.LOL");
	}
...
void CFxBossRaidHybri1_02::DeleteRes()
{
}

//-- num 1 : assault
//--     2 : D Assault
bool CFxBossRaidHybri1_02::Render(EffectSort &effect_sort)
{
	//-- 사용 캐릭이 사라질 경우
	if(!GET_D3DDEVICE() )
		return true;
	...
	if ( effect_sort.nCurFrame >= 20 && effect_sort.nCurFrame < 70)
	{
		...
		float fTime = effect_sort.nCurFrame - 20.0f;
		...
		float fAlpha = 1.0f - (sqrt(fTime ) * 0.05f);
		DWORD a = (DWORD)(0xFF * fAlpha) & (0XFF);
		DWORD w = (a << 24) | (a << 16) | (a << 8) | (a);

		GET_D3DDEVICE()->SetTransform(D3DTRANSFORMSTATE_WORLD, &matWorld );

		int Length2 = vDiff.x * vDiff.x + vDiff.z * vDiff.z;

		if (Length2 < 1000 * 1000)
		{
	...
	if ( effect_sort.nCurFrame >= 28 && effect_sort.nCurFrame < 80)
	{
		D3DUtil_SetIdentityMatrix(matScale);
		D3DUtil_SetIdentityMatrix(matWorld);
		D3DUtil_SetIdentityMatrix(matTrans);
		...
		float fTime = effect_sort.nCurFrame - 28;
		D3DVECTOR vDiff = Normalize(effect_sort.vTargetPos - effect_sort.vCurPos) * 8.0f * (fTime+2.5);
		D3DVECTOR vDiff2 = effect_sort.vTargetPos - effect_sort.vCurPos;
		float theta = atan2(vDiff2.z, vDiff2.x);
		D3DVECTOR vPos = effect_sort.vCurPos + vDiff + D3DVECTOR(0, 18, 0);

		D3DUtil_SetRotateXMatrix(matTemp2, g_PI * -0.30f);
		Nk3DUtil_SetRotateCCWYMatrix(matRotate, theta );
		D3DMath_MatrixMultiply(matRotate, matTemp2, matRotate);

		D3DUtil_SetTranslateMatrix( matTrans, vPos );
		D3DMath_MatrixMultiply(matTemp1, matRotate, matTrans);
		matScale._11 = matScale._22 = matScale._33 = sqrt(fTime) * 0.2f + 0.8f;;
		D3DMath_MatrixMultiply(matWorld, matScale, matTemp1);
