...
void CFxBee::LoadRes(IndexedTexture *pIndexedTexture, CShadowRect *pShadowRect)
{}
void CFxBee::DeleteRes()
{}

BOOL CFxBee::Render(EffectSort &effect_sort)
{
	if( !GET_D3DDEVICE() )
		return TRUE;
	...
	if( g_pCapsyongTexture )
		lpSurface = g_pCapsyongTexture->GetSurfaceByIdx( g_EffectTextr[ EFF_FX_BEETAIL ] );
	...
	for (i= end_frame - 1; i >= start_frame; i--)
	{
		...
		matWorld._42 = vDrawPos.y + FX_BEE_HEIGHT;
	...
	if ( fDist <= 0.0f || fabsf(fDist) <= 10.0f)
	{
		...
		// nate 2004 - 7 - image
		if( g_pCapsyongTexture )
			lpSurface = g_pCapsyongTexture->GetSurfaceByIdx( g_EffectTextr[ EFF_FX_BEEHEAD ] );
		GET_D3DDEVICE()->SetTexture(0, lpSurface);
