...
const int male_frame = 51;
const int fmale_frame = 42;

LHFX_Perom_Shotgun::LHFX_Perom_Shotgun(void)
	:	m_lolo(NULL)
{
}
...
void LHFX_Perom_Shotgun::LoadRes()
{
	if (!m_lolo)
	{
		m_lolo = new CLolos[2];
		m_lolo[0].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[0].Read("data/effect/lolos/pm_skill_shotgun.lol");
		m_lolo[1].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[1].Read("data/effect/lolos/pw_skill_shotgun.lol");
	}
}

void LHFX_Perom_Shotgun::DeleteRes()
{
}

bool LHFX_Perom_Shotgun::Render(EffectSort &effect_sort)
{
