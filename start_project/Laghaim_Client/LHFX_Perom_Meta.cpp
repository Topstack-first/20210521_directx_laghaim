...
const int male_frame = 41;
const int fmale_frame = 36;


LHFX_Perom_Meta::LHFX_Perom_Meta(void)
	:	m_lolo(NULL)
{
}
...
void LHFX_Perom_Meta::LoadRes()
{
	if (!m_lolo)
	{
		m_lolo = new CLolos[2];
		m_lolo[0].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[0].Read("data/effect/lolos/pm_skill_meta.lol");
		m_lolo[1].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[1].Read("data/effect/lolos/pw_skill_meta.lol");
	}
}

void LHFX_Perom_Meta::DeleteRes()
{
}

bool LHFX_Perom_Meta::Render(EffectSort &effect_sort)
{
