...
const int fmale_frame = 51;

LHFX_Perom_Thrust::LHFX_Perom_Thrust(void)
	:	m_lolo(NULL)
{
}
...
void LHFX_Perom_Thrust::LoadRes()
{
	if (!m_lolo)
	{
		m_lolo = new CLolos[2];
		m_lolo[0].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[0].Read("data/effect/lolos/pm_skill_thrust.lol");
		m_lolo[1].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[1].Read("data/effect/lolos/pw_skill_thrust.lol");
	}
}

void LHFX_Perom_Thrust::DeleteRes()
{
}

bool LHFX_Perom_Thrust::Render(EffectSort &effect_sort)
{
