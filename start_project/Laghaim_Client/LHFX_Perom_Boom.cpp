...
LHFX_Perom_Boom::LHFX_Perom_Boom(void)
	:	m_lolo(NULL)
{
}
...
void LHFX_Perom_Boom::LoadRes()
{
	if (!m_lolo)
	{
		m_lolo = new CLolos[2];
		m_lolo[0].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[0].Read("data/effect/lolos/pm_skill_boom.lol");
		m_lolo[1].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[1].Read("data/effect/lolos/pw_skill_boom.lol");
	}
}

void LHFX_Perom_Boom::DeleteRes()
{
}

bool LHFX_Perom_Boom::Render(EffectSort &effect_sort)
{
