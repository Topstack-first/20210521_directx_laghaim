...
const int fmale_frame = 38;

LHFX_Perom_Pierce::LHFX_Perom_Pierce(void)
	:	m_lolo(NULL)
{
}
...
void LHFX_Perom_Pierce::LoadRes()
{
	if (!m_lolo)
	{
		m_lolo = new CLolos[2];
		m_lolo[0].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[0].Read("data/effect/lolos/pm_skill_pierce.lol");
		m_lolo[1].SetIndexedTexture( g_pCapsyongTexture );
		m_lolo[1].Read("data/effect/lolos/pw_skill_pierce.lol");
	}
}

void LHFX_Perom_Pierce::DeleteRes()
{
}

bool LHFX_Perom_Pierce::Render(EffectSort &effect_sort)
{
