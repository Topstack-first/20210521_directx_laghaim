...
int do_extract_currentfile(unzFile uf, const char *target_dir,
						   const int* popt_extract_without_path, int* popt_overwrite)
	//unzFile uf;
	//const int* popt_extract_without_path;
    //int* popt_overwrite;
{
	char filename_inzip[1024];
	char filename_inzip_temp[256];
	...
	unz_file_info file_info;
	uLong ratio=0;
	err = unzGetCurrentFileInfo(uf,&file_info,filename_inzip_temp,sizeof(filename_inzip),NULL,0,NULL,0);
	...
	if (target_dir)
		sprintf(filename_inzip, "%s%s", target_dir, filename_inzip_temp);
	else
		strcpy(filename_inzip, filename_inzip_temp);
...
int do_extract(unzFile uf, const char *target_dir, int opt_extract_without_path,
    ...
    //int opt_overwrite;
{
	...
	for (i=0;i<gi.number_entry;i++)
	{
        if (do_extract_currentfile(uf, target_dir, &opt_extract_without_path,
    ...
    //int opt_overwrite;
{
    ...
    if (do_extract_currentfile(uf, NULL, &opt_extract_without_path,
                                      &opt_overwrite) == UNZ_OK)
        return 0;
    else
        return 1;
